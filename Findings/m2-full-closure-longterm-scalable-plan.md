...
- [-] Subtree scoping (`--dir src/`, focus on a module + its dependents) works end-to-end with proportional cost.  **(A2 early: simple directory filter implemented in Python-primary run_full_update + _collect + exposed in MCP/CLI --python-primary; prunes at candidate collection for early proportional cost. Foundation for full streaming.)**
- [ ] Shell parity maintained (thin wrapper calls Python streaming where possible).
- [-] **A2 early scaffolding complete (this slice)**: Partial result collection + continuation hints, basic progress dicts + phases, interrupt/budget safety, usable trustworthy partials (even on KeyboardInterrupt or max_files), all in Python-primary path + MCP update_maps (directory + max_files params + result fields). Directly attacks highest user-visible M2 blocker. See cli.py:run_full_update + mcp/server.py:UpdateMapsResult.
