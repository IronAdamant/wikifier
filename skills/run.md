...
**Human investigation layer (secondary, opt-in)**: `wikifier init` now copies `index.html` + `diagnostics.html` into the target project (the folder where MCP/CLI run). Humans open the local HTML in a browser to visually explore the agent wiki (health matrix, Mermaid tree diagram of the codebase, file summaries "what things look like"). It polls for auto-reload (tied to monitor). Copy buttons export text (Mermaid source, health table, full snapshot with summaries) for pasting to LLMs or team investigation — exactly the token-saving summaries agents use internally. The .md files + MCP/tools remain the primary for agents. This sub-project/layer lets humans make use of the wiki for their own work without changing agent behavior.
