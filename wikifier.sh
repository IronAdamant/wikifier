    ...
    if os.environ.get("WIKIFIER_DEBUG") or os.environ.get("DEBUG"):
        try:
            for r in (reps or [])[:8]:  # bounded for scale
                ...
                print(f"DEBUG BarrelReport: importer={imp} via_barrels=[{trig}] chains=[{cids}] reason={rsn} detector={det} v1={r.get('node_identity_version')}", file=sys.stderr)
        except Exception:
            pass
...
cmd_record_deletion() {
    ...
    # Delegate health + pending to locked Python (fixes dup / race / MD-vs-JSON inconsistency)
