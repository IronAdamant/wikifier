...
RESERVED_TOP_LEVEL_KEYS: Dict[str, str] = {
    "_reverse_dependencies": "Core reverse-dependency map (target -> [sources]). Always authoritative. A1: now first-class persisted index (parallel to forward graph + BRC). Incrementally maintained with O(changed) cost + own signature.",
    "_reverse_signature": "A1 (Workstream A1): short stable hash of the reverse dep index (target->sources lists). Enables delta detection, integrity, and observability parallel to _graph_signature. Auto-maintained by set_reverse_dependencies + incremental maintain helpers in import_cache. Populated on every update path (sh/python-primary).",
